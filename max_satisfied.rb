...
def max_satisfied(customers, grumpy, minutes)
  ...
  max_satisfied = 0
  grumpy_satisfied = 0
  grumpy.each_with_index do |g, i|
    if g == 0
      satisfied += customers[i]
    else
      grumpy_satisfied += customers[i]
    end
    if i >= minutes
      grumpy_satisfied -= customers[i - minutes] * grumpy[i - minutes]
    end
    max_satisfied = [max_satisfied, grumpy_satisfied].max
  end
  satisfied + max_satisfied
end

puts max_satisfied([1,0,1,2,1,1,7,5], [0,1,0,1,0,1,0,1], 3) # 16
puts max_satisfied([1], [0], 1) # 1